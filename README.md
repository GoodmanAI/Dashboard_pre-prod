...
Plain textANTLR4BashCC#CSSCoffeeScriptCMakeDartDjangoDockerEJSErlangGitGoGraphQLGroovyHTMLJavaJavaScriptJSONJSXKotlinLaTeXLessLuaMakefileMarkdownMATLABMarkupObjective-CPerlPHPPowerShell.propertiesProtocol BuffersPythonRRubySass (Sass)Sass (Scss)SchemeSQLShellSwiftSVGTSXTypeScriptWebAssemblyYAMLXML`   dotenvCopier# PostgreSQL  POSTGRES_USER=your_postgres_user  POSTGRES_PASSWORD=your_postgres_password  POSTGRES_DB=your_database_name  DATABASE_URL=postgresql://your_postgres_user:your_postgres_password@db:5432/your_database_name?schema=public  # JWT secret pour NextAuth  JWT_SECRET=your_jwt_secret  # Clé API Admin pour les routes protégées  ADMIN_API_KEY=your_admin_api_key  # Configuration SMTP (pour l'envoi d'e-mails via nodemailer)  SMTP_USER=your_smtp_user  SUPPORT_EMAIL=your_support_email@example.com   `
...
1.  bashCopiergit clone https://github.com/GoodmanAI/Dashboard\_pre-prod.gitcd Dashboard\_pre-prod
    
2.  bashCopierdocker-compose up --buildCette commande va :
...
3.  Une fois les conteneurs lancés, ouvrez votre navigateur et allez sur [http://localhost:3000](http://localhost:3000).
    

Commandes Utiles
----------------

*   bashCopierdocker-compose up --build
    
*   bashCopierdocker-compose down
    
*   bashCopierdocker-compose exec web npx prisma migrate deploy
    
*   bashCopierdocker-compose exec web npm run seed
...
Plain textANTLR4BashCC#CSSCoffeeScriptCMakeDartDjangoDockerEJSErlangGitGoGraphQLGroovyHTMLJavaJavaScriptJSONJSXKotlinLaTeXLessLuaMakefileMarkdownMATLABMarkupObjective-CPerlPHPPowerShell.propertiesProtocol BuffersPythonRRubySass (Sass)Sass (Scss)SchemeSQLShellSwiftSVGTSXTypeScriptWebAssemblyYAMLXML`   bashCopiernpm install  npm run dev   `

Vérifiez que la variable DATABASE\_URL dans votre .env est correctement configurée pour se connecter à votre instance PostgreSQL locale.