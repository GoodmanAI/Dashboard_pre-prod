...
**18 champs depuis le 18/09/2026.** Les 14 d'origine, les deux couleurs, les deux noms
d'expéditeur.

**Quatre champs ont été retirés le 18/09/2026** : `rappels_actifs`, `annulation_directe`,
`sms_rappel_mode` et `code_caracteristique_confirmation_xplore`. Ils pilotaient la
confirmation par lien, l'annulation directe et les rappels J-N de Konnect, tous
supprimés : plus aucune brique ne les lisait. Les relances no-show sont celles
d'AI2Xplore et se règlent dans leur propre rubrique (`SmsConfirmationConfig`), partagée
avec LyraeTalk. Les quatre colonnes restent dans `KonnectSettings`, sans lecteur : pas de
migration destructive. Konnect ignore ces clés si un Dashboard ancien les envoie encore.

Deux champs récents sont `couleur_principale` et `couleur_secondaire` `couleur_principale` et `couleur_secondaire`
...
Les deux derniers sont `expediteur_nom_mail` et `expediteur_sms`
